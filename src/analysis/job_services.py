"""Job filtering, insight, report, and upload helpers shared by every interface.

This module deliberately imports no web framework. The FastAPI service and the
Streamlit dashboard both build on it, so nothing here may depend on either one.
Streamlit-specific caching lives in src/dashboard/services.py.
"""
